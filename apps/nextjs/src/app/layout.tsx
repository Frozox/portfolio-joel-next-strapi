...
export const generateMetadata = async (): Promise<Metadata> => {
  return{
    ...
    description: 'Joël Chapeau est un artiste plasticien contemporain français dont les œuvres explorent la mémoire, la matière et l’abstraction à travers des techniques mixtes.',
    applicationName: 'Portfolio de Joel Chapeau',
    openGraph: {
      images: new URL(`${env.NEXT_PUBLIC_FRONTEND_HOST}/og-image.jpg`),
      description: 'Joël Chapeau est un artiste plasticien français dont le travail mêle abstraction, mémoire et matière. À travers la peinture, l’assemblage et l’expérimentation, il explore la trace, le geste et le temps, proposant une œuvre profondément sensorielle et introspective.',
      title: 'Joël Chapeau',
      url: new URL(env.NEXT_PUBLIC_FRONTEND_HOST)
    },
    authors: [{ name: 'Joël Chapeau' }, { name: 'Tom Cuillandre' }],
    generator: 'Next.js',
    keywords: ['joel chapeau', 'joel artiste', 'joel peinture', 'joel sculpture', 'artiste plasticien', 'artiste grenade'],
...
const RootLayout = async ({ children }: Readonly<TLayoutProps>) => {
  ...
  await prefetchArtCategories(queryClient, { sort: 'order', populate: 'image' });
