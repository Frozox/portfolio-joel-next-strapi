  ...
  return (
    <li>
      ...
      <ul className='mt-4 space-y-1 text-xs sm:text-base'>
        { React.Children.map(props.children, (children) => <li>{children}</li>)}
      </ul>
    </li>
  );
});
        ...
        <div className='w-full justify-end md:flex lg:justify-center'>
          <ul className="grid grid-cols-3 gap-8 lg:grid-cols-3">
            ...
            {...artCategories.map(artCategory => (
              <Link key={artCategory.attributes.slug} href={artCategory.attributes.slug}>
                <span>{artCategory.attributes.name}</span>
              </Link>
            ))}
            </FooterElement>
            <FooterElement title='Réseaux'>
              <Link rel="noopener noreferrer nofollw external" href={'https://www.instagram.com/joelchapeau/'} className='flex space-x-2' target='_blank'>
                <InstagramIcon />
